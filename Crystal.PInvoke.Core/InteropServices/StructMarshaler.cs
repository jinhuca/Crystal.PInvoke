		...
		private StructMarshaler(string cookie)
		{
			...
			if (!string.IsNullOrEmpty(cookie))
			{
				...
				if (st is null || !typeof(IMemoryMethods).IsAssignableFrom(st))
					throw new InvalidOperationException("Invalid string in cookie. It must be a string identifier of a type that derives from Crystal.InteropServices.IMemoryMethods.");
				t = st;
			}
