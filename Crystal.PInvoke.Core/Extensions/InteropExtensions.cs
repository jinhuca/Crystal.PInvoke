...
using Crystal.PInvoke.Collections;
...
namespace Crystal.Extensions
{
	...
	public static partial class InteropExtensions
	{
		...
		public static bool IsMarshalable(this Type type)
		{
			...
			return t.IsSerializable || CrystalMarshaler.CanMarshal(t, out _) || t.IsBlittable();
		}
		...
		[Obsolete("Please use the Crystal.Extensions.InteropExtensions.Write method instead. This will be removed from the library shortly as it performs no allocation.", true)]
		...
		{
			memLock ??= Passthrough;
			if (CrystalMarshaler.CanMarshal(typeof(T), out ICrystalMarshaler marshaler))
			{
		...
		{
			if (CrystalMarshaler.CanMarshal(type, out ICrystalMarshaler marshaler))
		...
		internal static object GetValueType(IntPtr ptr, Type type, Type trueType = null, int offset = 0, SizeT allocatedBytes = default)
		{
			...
			var obj = CrystalMarshaler.CanMarshal(trueType, out ICrystalMarshaler marshaler) ?
		...
		internal static int WriteNoChecks(IntPtr ptr, object value, int offset, SizeT allocatedBytes)
		{
			...
			if (CrystalMarshaler.CanMarshal(valType, out ICrystalMarshaler marshaler))
			{
				using SafeAllocatedMemoryHandle mem = marshaler.MarshalManagedToNative(value);
